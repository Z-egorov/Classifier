...
os.chdir("..")
path = os.getcwd()
zeroing.zeroing_file(path + '/engine/result.txt')
zeroing.zeroing_folder(path + '/engine/task1')


folder = Path(path + '/engine/data/docs/')
theme = Path(path + '/engine/data/themes/')
...
for file_doc in folder.iterdir():  # перебор файлов  # перебор файлов
    ...
    f = open(path + '/engine/task1/'+ str(os.path.basename(file_doc.name)) + '.txt', 'w+', encoding='utf-8')
    ...
    result = open(path + '/engine/result.txt', 'a', encoding='utf-8')
